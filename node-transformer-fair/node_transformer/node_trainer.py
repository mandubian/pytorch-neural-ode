...
class Trainer(object):
    ...
    def init_meters(self, args):
        ...
        self.meters['nfe_encoder'] = AverageMeter()    # Node encoder
        self.meters['nfe_decoder'] = AverageMeter()    # Node Decoder
        if args.fp16:
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        try:
            ...
            if self._model.has_node_encoder:
                self.meters['nfe_encoder'].update(self._model.encoder.nfe())
                self._model.encoder.reset_nfe()
            if self._model.has_node_decoder:
                self.meters['nfe_decoder'].update(self._model.decoder.nfe())
                self._model.decoder.reset_nfe()
